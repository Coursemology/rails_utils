  ...
  module ActionViewExtensions
    ...
    def flash_messages(options = {})
      flash.collect do |key, message|
        ...
        content_tag(:div, content_tag(:button, options[:button_html] || "x", type: "button", class: options[:button_class] || "close", "data-dismiss" => "alert") + strip_tags(message), class: "#{flash_class(key)} fade in #{options[:class]}")
      end.join("\n").html_safe
    end
